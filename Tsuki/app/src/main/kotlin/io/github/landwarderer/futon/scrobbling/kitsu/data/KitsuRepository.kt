package io.github.landwarderer.futon.scrobbling.kitsu.data
...
import io.github.landwarderer.futon.core.util.ext.parseJsonOrNull
...
import io.github.landwarderer.futon.scrobbling.common.domain.model.ScrobblerMangaInfo
...
import io.github.landwarderer.futon.scrobbling.kitsu.data.KitsuInterceptor.Companion.VND_JSON
...
class KitsuRepository(
        ...
        override val oauthUrl: String = "tsuki+kitsu://auth"
        ...
        override suspend fun loadUser(): ScrobblerUser {
                ...
                val response = okHttp.newCall(request.build()).await().parseJson()
                        .getJSONArray("data")
                        .getJSONObject(0)
                val attributes = response.getJSONObject("attributes")
                return ScrobblerUser(
                        id = response.getAsLong("id"),
                        nickname = attributes.getString("name"),
                        avatar = attributes.optJSONObject("avatar")?.getStringOrNull("small"),
                        coverImage = attributes.optJSONObject("coverImage")?.let {
                                it.getStringOrNull("original") ?: it.getStringOrNull("large")
                        },
