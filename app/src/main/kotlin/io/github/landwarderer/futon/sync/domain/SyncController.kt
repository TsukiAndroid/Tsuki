...
class SyncController @Inject constructor(
        ...
        // Shared with AppUpdateRepository — written here so the migration banner can detect
        // "user had sync configured" even if Android auto-removed the orphaned account on update.
        private val migrationPrefs = context.getSharedPreferences(SYNC_MIGRATION_PREFS, Context.MODE_PRIVATE)
        ...
        private fun peekAccount(): Account? {
                ...
                if (account != null && !migrationPrefs.getBoolean(KEY_HAD_SYNC_ACCOUNT, false)) {
                        migrationPrefs.edit().putBoolean(KEY_HAD_SYNC_ACCOUNT, true).apply()
                }
                return account
        }
        ...
        companion object {
                ...
                // Must match constants in AppUpdateRepository exactly — both classes share this prefs file.
                internal const val SYNC_MIGRATION_PREFS = "sync_migration"
                internal const val KEY_HAD_SYNC_ACCOUNT = "had_sync_account"
        }
}
